...
class CryptSM2(object):
    ...
    def gen_k_by_RFC6979(self, data):  # 生成随机数k的函数，data消息的hash，private_key私钥均为16进制字符串
        v = b'\x01' * 32
        k = b'\x00' * 32
        priv = bytes(self.private_key, encoding='utf-8')
        msghash = data
        k = hmac.new(k, v + b'\x00' + priv + msghash, hashlib.sha256).digest()
        v = hmac.new(k, v, hashlib.sha256).digest()
        k = hmac.new(k, v + b'\x01' + priv + msghash, hashlib.sha256).digest()
        v = hmac.new(k, v, hashlib.sha256).digest()
        return hex(int.from_bytes(hmac.new(k, v, hashlib.sha256).digest(), "big") % int(self.ecc_table['n'], base=16))[
               2:]

    def sign(self, data):  # 签名函数, data消息的hash，private_key私钥，均为16进制字符串
        K = self.gen_k_by_RFC6979(data)
