...
def PGP_Sender(msg):
    ...
    Enc_key = SM2_encrypt(symmetric_key)
    SM4_encrypt = CryptSM4()
    SM4_encrypt.set_key(symmetric_key, SM4_ENCRYPT)
    Enc_msg = SM4_encrypt.crypt_ecb(msg)
    return Enc_msg, Enc_key


def PGP_Receiver(Enc_msg, Enc_key):
    symmetric_key = SM2_decrypt(Enc_key)
    SM4_encrypt = CryptSM4()
    SM4_encrypt.set_key(symmetric_key, SM4_DECRYPT)
    message = SM4_encrypt.crypt_ecb(Enc_msg)
    return message
...
if __name__ == '__main__':
    ...
    enc_m, enc_key = PGP_Sender(m)
    rev_m = PGP_Receiver(enc_m, enc_key)
    rev_m=rev_m.decode('utf8')
    print(rev_m)
